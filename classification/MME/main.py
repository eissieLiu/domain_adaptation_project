...
# Training settings
parser = argparse.ArgumentParser(description='CS591 MME')
...
args.batch_size = 24
args.img_size = 32
args.model_name = args.source + '2' + args.target + '_' + args.net + '.pth.tar'
...
record_file = os.path.join(record_dir,'%s_net_%s_%s_to_%s_num_%s' %
...
def train():
    ...
    for step in range(all_step):
        ...
        if step % args.log_interval == 0:
            print(log_train)
        if (step % args.save_interval == 0 and step > 0) or step == 10:
            loss_val, acc_val = test(target_loader_val, 0)
            loss_test, acc_test = test(target_loader_test, step)
            G.train()
            F1.train()
            if acc_val >= best_acc:
                best_acc = acc_val
...
def test(loader, tp):
    ...
    with torch.no_grad():
        for batch_idx, data_t in enumerate(loader):
            ...
            if cnt < 20:
                ...
                all_label.append(gt_labels_t.cpu())
            output1 = F1(feat)
    ...
    if tp == 10 or tp == 2000:
        feat_in_one = torch.cat(all_feat, 0)
        label_in_one = torch.cat(all_label, 0)
        feat_in_one = np.array(feat_in_one)
        label_in_one = np.array(label_in_one)
        np.save('%d.npy' % tp, feat_in_one)
        np.save('%d_label.npy' % tp, label_in_one)
